...
class GridWorld:
    ...
    def __init__(
        ...
        for state in range(self.get_grid_space()):
            if self._is_lava_state(self._state_list[state]):
                self._lava_states.append(self._state_list[state])

        if len(self._lava_states) > 0:
            # get the leftest coordinate of lava states
            min_y = min(self._lava_states, key=lambda x: x[1])[1]
    ...
    def step(self, action: int) -> tuple:
        ...
        if self.stateMap.get(state_coord) is None:
            if self._is_lava_state(state_coord):
                self.stateMap[state_coord] = "lava"
                return (None, self.step_reward, True, trucated)
            elif self._is_exit_state(state_coord):
                self.stateMap[state_coord] = "exit"
                return (None, self._exit_reward, True, trucated)
            elif self._is_trap_state(state_coord):
                self.stateMap[state_coord] = "trap"
                return (None, self._trap_reward, True, trucated)
            elif self._is_goal_state(state_coord):
                self.stateMap[state_coord] = "goal"
                return (None, self._goal_reward, True, trucated)
            elif self._is_bait_state(state_coord):
                self.stateMap[state_coord] = "bait"
                self.bite()
            elif self._is_closed and self._is_key_state(state_coord):
                self.stateMap[state_coord] = "key"   
                self.open_door()
            else:
                self.stateMap[state_coord] = "None"
        else:
            if self.stateMap[state_coord] == "goal":
                return (None, self._goal_reward, True, trucated)
            elif self.stateMap[state_coord] == "trap":
                return (None, self._trap_reward, True, trucated)
            elif self.stateMap[state_coord] == "lava":
                return (None, self.step_reward, True, True)
            elif self.stateMap[state_coord] == "exit":
                return (None, self._exit_reward, True, trucated)
            elif self.stateMap[state_coord] == "bait" and not self._is_baited:
                self.bite()  
            elif self.stateMap[state_coord] == "key" and self._is_closed:
                self.open_door() 
            
        if self.model.get((state_coord, action)) is None:
            next_state_coord = self._get_next_state(state_coord, action)
            next_state = self._state_list.index(next_state_coord)
            self.model[(state_coord, action)] = next_state
        else:
            next_state = self.model[(state_coord, action)]
        ...
        if self.stateMap.get(self._current_state) is None:
            if self._is_bait_state(self._state_list[self._current_state]):
                self.stateMap[self._state_list[self._current_state]] = "bait"
                return (next_state, self._bait_reward, False, trucated)
        elif self.stateMap[self._current_state] == "bait":
            return (next_state, self._bait_reward, False, trucated)
        return (next_state, self.step_reward, False, trucated)
